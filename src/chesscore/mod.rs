...
impl PieceProps {
    fn new(jump_offsets: Option<Vec<(Delta, Delta)>>, slide_directions: Option<Vec<(Delta, Delta)>>) -> Self {
...
impl PromotionProps{
    pub fn default()->Self{
        PromotionProps{ 
            white_promotion_squares: HashMap::from([(0,true),(1,true),(2,true),(3,true),(4,true),(5,true),(6,true),(7,true)]), 
            black_promotion_squares: HashMap::from([(56,true),(57,true),(58,true),(59,true),(60,true),(61,true),(62,true),(63,true)]), 
...
impl VariantActions for Variant{
    ...
    fn get_pseudo_legal_moves(&self, color: Color) -> Vec<Move> {
        match self{
            Variant::Checkmate(checkmate_variant)=> checkmate_variant.get_pseudo_legal_moves(color),
            Variant::AntiChess(antichess_variant) => antichess_variant.get_pseudo_legal_moves(color),
...
impl VariantActions for DefaultVariant{
            ...
            .filter(|mv| {
                    if mv.classic_move_type==ClassicMoveType::Capture {
                        if let Some(piece) = self.position.piece_locations.get(&mv.dest){
                            if piece.piece_type == PieceType::King { return false}
                        }
                    }
                    self.make_move(mv);
    ...
    fn get_pseudo_legal_moves(&self, color: Color) -> Vec<Move> {
        ...
        for (sq,piece) in &self.position.piece_locations{
            if color!= piece.player{ continue;}
...
impl DefaultVariant{
            ...
            for i in -1..=1{
                if i==0{
                    ...
                    if self.position.is_sq_empty(&t1){
                        moves.push(Move { src: *sq, dest: t1, classic_move_type: ClassicMoveType::Quiet, variant_move_type: None, piece: piece.clone()})
                    } 
                    if let Ok(target_row_2) = self.position.add_delta_row(target_row,row_offset){
                        let t2 = self.position.to_pos(&target_row_2,&src.1);
                        if self.position.is_sq_empty(&t1) && self.position.is_sq_empty(&t2) && src.0 == double_start_rank{
                            moves.push(Move { src: *sq, dest: t2, classic_move_type: ClassicMoveType::DoublePawnPush, variant_move_type: None, piece: piece.clone() })
                        }
                    }
                } else{
                    if let Ok(target_col) = self.position.add_delta_col(src.1,i as isize) {
                        ...
                        if let Some(opp) = self.position.piece_locations.get(&target){
                            if !self.is_opp_king(piece.player, opp){
                                moves.push(Move { src: *sq, dest: target, classic_move_type: ClassicMoveType::Capture, variant_move_type: None, piece: piece.clone() })
                            }
                        } else if self.position.is_sq_empty(sq) && self.position.ep_square == Some(target){
                            moves.push(Move { src: *sq, dest: target, classic_move_type: ClassicMoveType::EnPassant, variant_move_type: None, piece: piece.clone() })
                        }
                    }
    ...
    #[inline]
    fn total_sq(&self)->u32{  self.position.dimensions.ranks * self.position.dimensions.files - 1 }
    ...
    fn get_attacked_squares(&self,moves:Vec<Move>)->HashMap<Square,bool>{
        ...
        for mv in moves{
            match mv.classic_move_type{
                ...
                ClassicMoveType::Capture => {attacked_squares.insert(mv.dest, true);},
                _ =>{}
            }
        }
        attacked_squares
            ...
            if let Some(mut target) = self.get_target(sq, offset){
                while target< self.total_sq(){
                    match self.position.piece_locations.get(&target){
                        ...
                        None =>  moves.push(Move { src: *sq, dest: target, classic_move_type: ClassicMoveType::Quiet, variant_move_type: None, piece: piece.clone() })
                    }
                    let new_target = self.get_target(&target, offset);
                    if new_target.is_none(){ break;} else { target = new_target.unwrap()}
                }
            }
...
impl AntichessVariant{
    ...
    fn get_legal_moves(&mut self) -> Vec<Move> {
        ...
        .iter().cloned()
        .filter(|mv| mv.classic_move_type == ClassicMoveType::Capture).collect();
